...
;layui.define(["laytpl", "laypage", "layer", "form"], function (e) {
    "use strict";
    var t = layui.$, i = layui.laytpl, a = layui.laypage, l = layui.layer, n = layui.form, d = layui.hint(), c = layui.device(), r = {
    ...
    }, s = "table", u = ".layui-table", f = "layui-hide", h = "layui-table-view", y = ".layui-table-header", p = ".layui-table-body", v = ".layui-table-main", m = ".layui-table-fixed", x = ".layui-table-fixed-l", b = ".layui-table-fixed-r", g = ".layui-table-tool", k = ".layui-table-sort", C = "layui-table-edit", w = "layui-table-hover", z = function (e) {
        ...
        }(), "{{# if(item2.checkbox){ }}", '<th data-field="{{ item2.field||i2 }}" data-type="checkbox" unresize="true"><div class="layui-table-cell laytable-cell-checkbox"><input type="checkbox" name="layTableCheckbox" lay-skin="primary" lay-filter="layTableAllChoose" {{# if(item2[d.data.checkName]){ }}checked{{# }; }}></div></th>', "{{# } else { }}", '<th data-field="{{ item2.field||i2 }}" {{#if(item2.colspan){}} colspan="{{item2.colspan}}"{{#} if(item2.rowspan){}} rowspan="{{item2.rowspan}}"{{#}}} {{# if(item2.unresize){ }}unresize="true"{{# } }}>', "{{# if(item2.colspan > 1){ }}", '<div class="layui-table-cell laytable-cell-group" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>', '<span>{{item2.title||""}}</span>', "</div>", "{{# } else { }}", '<div class="layui-table-cell laytable-cell-{{d.index}}-{{item2.field||i2}}" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>', '<span>{{item2.title||""}}</span>', "{{# if(item2.sort){ }}", '<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc"></i><i class="layui-edge layui-table-sort-desc"></i></span>', "{{# } }}", "</div>", "{{# } }}", "</th>", "{{# }; }}", e.fixed ? "{{# }; }}" : "", "{{# }); }}", "</tr>", "{{# }); }}", "</thead>", "</table>"].join("")
    }, A = ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" ', '{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>', "<tbody></tbody>", "</table>"].join(""), T = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">', "{{# var fixed, right; }}", '<div class="layui-table-header">', z(), "</div>", '<div class="layui-table-body layui-table-main">', A, "</div>", '{{# if(fixed && fixed !== "right"){ }}', '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', z({fixed: !0}), "</div>", '<div class="layui-table-body">', A, "</div>", "</div>", "{{# }; }}", "{{# if(right){ }}", '<div class="layui-table-fixed layui-table-fixed-r">', '<div class="layui-table-header">', z({fixed: "right"}), '<div class="layui-table-mend"></div>', "</div>", '<div class="layui-table-body">', A, "</div>", "</div>", "{{# }; }}", "{{# if(d.data.page){ }}", '<div class="layui-table-tool">', '<div class="layui-inline layui-table-page" id="layui-table-page{{d.index}}"></div>', "</div>", "{{# } }}", "<style>", "{{# layui.each(d.data.cols, function(i1, item1){", "layui.each(item1, function(i2, item2){ }}", ".laytable-cell-{{d.index}}-{{item2.field||i2}}{ width:{{item2.width||50}}px }", "{{# });", "}); }}", "</style>", "</div>"].join(""), D = t(window), F = t(document), j = function (e) {
        var i = this;
    ...
    j.prototype.config = {limit: 30, loading: !0}, j.prototype.render = function () {
        ...
        var l = a.elem, n = l.next("." + h), d = e.elem = t(i(T).render({VIEW_CLASS: h, data: a, index: e.index}));
        a.index = e.index, n[0] && n.remove(), l.after(d), e.layHeader = d.find(y), e.layMain = d.find(v), e.layBody = d.find(p), e.layFixed = d.find(m), e.layFixLeft = d.find(x), e.layFixRight = d.find(b), e.layTool = d.find(g), e.pullData(1), e.events()
        ...
        var o = this, s = e.data, u = o.config, f = [], h = [], y = [], p = function () {
            return !c && o.sortKey ? o.sort(o.sortKey.field, o.sortKey.sort, !0) : (layui.each(s, function (e, a) {
                var l = [], n = [], d = [];
                o.eachCols(function (c, o) {
                    var s = a[o.field || c] || (0 === c ? e + 1 : "");
                    if (!(o.colspan > 1)) {
                        var f = ['<td data-field="' + (o.field || c) + '"' + function () {
                            var e = [];
                            return o.edit && e.push(' data-edit="true"'), o.align && e.push(' data-off="click" align="' + o.align + '"'), o.templet && e.push(' data-content="' + s + '"'), o.event && e.push(' lay-event="' + o.event + '"'), o.style && e.push(' style="' + o.style + '"'), e.join("")
                        }() + ">", '<div class="layui-table-cell laytable-cell-' + function () {
                            return o.checkbox ? "checkbox" : u.index + "-" + (o.field || c)
                        }() + '">' + function () {
                            return o.checkbox ? '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" ' + function () {
                                var e = r.config.checkName;
                                return o[e] ? (a[e] = o[e], o[e] ? "checked" : "") : a[e] ? "checked" : ""
                            }() + ">" : "right" === o.fixed && o.toolbar ? t(o.toolbar).html() : o.templet ? i(t(o.templet).html() || String(s)).render(a) : s
                        }(), "</div></td>"].join("");
                        l.push(f), o.fixed && "right" !== o.fixed && n.push(f), "right" === o.fixed && d.push(f)
                    }
        ...
        if (o.key = u.id || u.index, r.cache[o.key] = s, c)return p();
        if (o.cacheData = s, u.height) {
            var v = parseFloat(u.height) - parseFloat(o.layHeader.height()) - 1;
            u.page && (v -= parseFloat(o.layTool.outerHeight() + 2)), o.layBody.css("height", v)
        }
        return 0 === s.length ? o.layMain.html('<div class="layui-none">无数据</div>') : (p(), void(u.page && (o.page = l, o.count = d, a.render({
        ...
        var e, a = this, d = a.config, o = t("body"), u = {}, f = a.layHeader.find("th"), h = ".layui-table-cell", y = d.id || d.elem.attr("lay-filter");
        f.on("mousemove", function (e) {
            ...
            var e = t(this).prev(), i = a.layBody.find('input[name="layTableCheckbox"]'), l = e.parents("tr").eq(0).data("index"), d = e[0].checked, c = "layTableAllChoose" === e.attr("lay-filter");
            ...
            var e, l = t(this), n = l.parent().data("field"), d = l.parents("tr").eq(0).data("index"), c = r.cache[a.key][d];
            ...
            var e = t(this), l = e.parents("tr").eq(0).data("index"), n = a.layBody.find('tr[data-index="' + l + '"]'), d = "layui-table-click";
